...
3. Query authoritative records, excluding COLLECTED/SOLD, still-hidden BURIED records and items already in the player's bag. For TABLE/BIN/SEALED items, guide to the owning station/bag/container with the remaining action instead of drawing hundreds of overlapping item markers.
...
**Acceptance:** `discovery_scan` tests newly unlocked material group, unknown definition refusal, hidden/revealed buried state, item carried to another zone, sorted-but-uncollected owner markers and zero remaining matches. Last-known-item search works without out-of-bounds/offscreen confusion.
