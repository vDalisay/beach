...
4. Add only a short native validation script with a direct boot check and explicit nonzero exit on failure. It instantiates/frees main without loading the entire vendor project. Do not set up case discovery, a test registry, a custom command interface or fixture infrastructure; validate working gameplay in scenes as it is added.
