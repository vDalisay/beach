...
Current status — 23 September 2026: [C00](../handoffs/C00.md) established a reproducible gameplay baseline at `f44a5bf` and the user reaffirmed Godot 4.6.1. [C01](../handoffs/C01.md) repaired normal-input collection and contextual carrying. [C02](../handoffs/C02.md) completed table focus and pause. [C03](../handoffs/C03.md) completed mixed bag order, three local checkpoints and result metadata. Visual alignment and release evidence remain incomplete. Continue with [C04–C09](07-completion-plan.md) using the [implementation and screenshot review](06-implementation-review.md).

The baseline commit contains project-owned implementation; licensed Synty source and generated staged art remain local and require the C00 staging steps. The committed requirements include the confirmed 160 m layout and current access-purchase prerequisites. C04–C07 specify layout/slot defaults, reef-before-clutter ordering, a visual rubric and repeatable paired-capture work; those instructions are not claims that the visual changes or capture option already exist.
