...
**Start here: [the finishing phase](11-finishing-phase.md) is the single remaining-work checklist.** Work through FIN-01–FIN-10: close systems first, finish visuals in their own part of the phase, then validate the final release. Every known gap and the revised look-and-feel work has an owner and a completion condition there. All ten boxes closed with evidence means the agreed release scope is done.

**UI/UX:** the [UI/UX pass](14-ui-ux-pass.md) (packets U00–U10) gives menus, HUD, prompts and text one look taken from the wordmark and the Synty interface packs: a live title screen over the beach, a panel-free HUD, input-icon prompts, frosted modal screens, animated buttons and 2D/3D reward bursts. Presentation only. Evidence goes to [U-ui](../handoffs/U-ui.md).

**Beach terrain and litter:** the [beach terrain pass](15-beach-terrain-pass.md) (packets B00–B05) gives the dry beach natural relief (dunes with dune grass, mounds and a berm, level under every gameplay place), rests every item on the ground (leaning with the slope, bottles and cans on their side) and lays the litter out the way tidy-up games do: hotspots with causes (picnics, the court, lounge rows, sandcastles, the tideline, the dune foot) and clean sand between them. B06 adds sand grain, wind ripples and scattered shells, and waves that run up the sand and leave it wet. Content version 9; evidence in [B-beach](../handoffs/B-beach.md).

**Game feel:** the [game-feel plan](12-game-feel.md) (packets J00–J14) specifies the hover, reticle, hands and tool animation, pickup and placement motion, and escalating completion effects that FIN-08 accepts. It covers the set shine, restoration bloom, collection coins and the postcard finale. It is presentation-only work: it changes no ownership, payment, completion, save or locked requirement, and adds no sound. Evidence goes to [J-feel](../handoffs/J-feel.md).

The [systems review](10-systems-review.md) records what is implemented and what remains unverified. The [C-packet specifications](07-completion-plan.md) retain supporting detail. The revised [look-and-feel specification](09-reference-look-plan.md), including **all V00–V07**, is incorporated into FIN-06–FIN-08, with final hardware acceptance in FIN-10. These are supporting documents, not separate backlogs. The [look review and low-poly model pass](12-look-and-model-pass.md) records the reference comparison, the palette/sky/palm changes, the integrated models for every asset role, and the follow-up city, reef-garden and FOV work under FIN-05/FIN-08. The [performance pass](13-performance-pass.md) records the measured frame, hitch and memory work, the Graphics settings presets and the profiling probe used for FIN-08/FIN-10.

Current status — 24 September 2026, reviewed gameplay `68a123a`: Godot 4.6.1, content/beach version 8, generator `manifest-1`, save schema 1. C00–C03a repairs are implemented; F13–F15 are closed in their recorded regressions. C05's six missing logical waste forms are present. C04 has occupied target/overlap and 15-family approach evidence; C07 has section fish and lounge/reef turtle routes; C08 has further focus/input/carry fixes. Fresh boot, sorting, save, economy, accelerated full completion and rendered traversal checks pass. The review reproduces **F16: a lost distinct press between physics samples** and **F17: hardcoded service interaction prompts**; new **C03b** owns both. Normal-play access/pacing, physical controller and current exported hardware/long-session acceptance remain open. Existing C09 packages predate current edits.

The baseline commit contains project-owned implementation; licensed Synty source and generated staged art remain local and require the C00 staging steps. The committed requirements include the confirmed 160 m layout and current access-purchase prerequisites. [C04's checkpoint](../handoffs/C04.md) includes grouped destinations, structural reef geometry and the working paired-capture option. Its visual rubric remains incomplete. Keep `601cee3` for C03-era content-5 saves, `3c6b431` for content-6 saves, `92fb167` for content-7 saves before C05's deterministic manifest change, and `768070b` for content-8 saves before the beach terrain pass.

The P00–P30 index below is the original implementation specification, not a current completion checklist. Its original dependencies do not override the finishing phase's execution order. The starting-point and planning-validation sections record the historical 22 September planning pass. Use [the systems reconciliation](10-systems-review.md#systems-reconciliation-and-remaining-acceptance) for reviewed status and the finishing checklist for live progress; the confirmed requirements remain authoritative. Consolidating the plan does not implement fixes or close acceptance gates.
...
Current execution priority: complete cleanup, organizing, progression, restoration, controls, saving and release verification; handle visual fidelity separately. Optional vehicles remain deferred. Target Windows, 60 fps on an 8 GB RAM / GTX 980-class system. Do not introduce Windows-only runtime dependencies that prevent a later Linux port.
