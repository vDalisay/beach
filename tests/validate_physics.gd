...
func _measure_full_scale() -> void:
	...
	if "--c04-dense" in OS.get_cmdline_user_args():
	...
	elif "--c04-pier" in OS.get_cmdline_user_args():
		player.global_position = Vector3(62.5, 3.1, 60)
		player.camera.look_at(Vector3(62.5, 3.0, 84))
		await _physics_frames(90)
		...
		await _physics_frames(90)
	elif "--c07-reef" in OS.get_cmdline_user_args():
		player.set_physics_process(false)
		session.swim_service.set_physics_process(false)
		player.global_position = Vector3(2.5, -2.9, 101)
		player.camera.look_at(Vector3(2.5, -2.25, 107.5))
		player.camera.environment = SwimService.UNDERWATER_ENVIRONMENT
		await _physics_frames(90)
	...
	if "--c04-dense" in OS.get_cmdline_user_args() or "--c04-pier" in OS.get_cmdline_user_args() or "--c06-water" in OS.get_cmdline_user_args() or "--c07-reef" in OS.get_cmdline_user_args():
		main.free()
		quit(failures)
		return
	var world_count := 0
